...
class DQNAgent:
    def __init__(self, action_size):
        ...
        self.summary_writer = tf.summary.FileWriter(
            'summary/boxing_dqn', self.sess.graph)
        self.sess.run(tf.global_variables_initializer())

        if self.load_model:
            self.model.load_weights("./save_model/boxing_dqn.h5")
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        # 1000 에피소드마다 모델 저장
        if e % 300 == 0:
            agent.model.save_weights("./save_model/seaquest_dqn_" + str(e) + ".h5")