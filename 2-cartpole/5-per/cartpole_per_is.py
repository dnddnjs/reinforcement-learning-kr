...
class DQNAgent:
    ...
    def train_model(self):
        ...
        for i in range(self.batch_size):
            ...
            is_weights.append(mini_batch[i][2])
...
class Memory:  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        ...
        priorities = self.tree.tree[self.tree.capacity - 1:]
        min_p = np.min(priorities[np.nonzero(priorities)])
        ...
        for i in range(n):
            ...
            is_weight = np.power(20000 * (p/self.tree.total()), -self.beta)
            is_weight_max = np.power(2000 * (min_p/self.tree.total()), -self.beta)
